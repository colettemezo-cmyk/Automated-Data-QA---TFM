"""Training workflow — stage 1: CSV to parquet."""
...
from workflows.training.config import CSV_PATH, PARQUET_PATH
...
def build_parquet_from_csv(
    ...
    if parquet_up_to_date and not parquet_is_readable(parquet_path):
        print(
            f"[training/ingest] {parquet_path} corrupted; re-parsing from CSV."
        )
        parquet_up_to_date = False

    if parquet_up_to_date:
        print(f"[training/ingest] {parquet_path} is up to date, skipping CSV parse.")
        derive_extra_features(parquet_path)
        return

    print(f"[training/ingest] reading {csv_path} ...")
    df = pl.read_csv(
        csv_path,
        schema_overrides={
    ...
    print(f"[training/ingest] wrote {parquet_path} ({df.height:,} rows, {df.width} cols).")
